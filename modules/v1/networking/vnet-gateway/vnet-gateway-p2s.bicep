...
resource publicIP02 'Microsoft.Network/publicIPAddresses@2020-06-01' = {
  name: '${publicIpNameVPN}-002'
  location: location
  sku: {
    name: 'Standard'
  }
...
resource publicIP03 'Microsoft.Network/publicIPAddresses@2020-06-01' = {
  name: '${publicIpNameVPN}-003'
  location: location
  sku: {
    name: 'Standard'
  }
...
resource publicIPDiagnostics02 'Microsoft.Insights/diagnosticsettings@2021-05-01-preview' = if (!empty(diagSettings)) {
  name: empty(diagSettings) ? 'dummy-value' : diagSettings.name
  scope: publicIP02
  properties: {
    workspaceId: empty(diagSettings.workspaceId) ? json('null') : diagSettings.workspaceId
